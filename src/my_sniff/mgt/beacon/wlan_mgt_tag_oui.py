...
class WLANMGTTagOUI(WLAN):
	...
	def wlan_mgt_tag_oui(self, packet):
	...
	# Only able to get the 1st value
	def wlan_mgt_tag_vendor_oui_type(self, packet):
		field_name = self.layer_name + '.' + 'tag.vendor.oui.type'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_marvell_ie_type(self, packet):
		field_name = self.layer_name + '.' + 'marvell.ie.typee'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_marvell_data(self, packet):
