...
class WLANMGTTagTIM(WLAN):
	...
	def wlan_mgt_tim_dtim_count(self, packet):
		field_name = self.layer_name + '.' + 'tim.dtim_count'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_tim_dtim_period(self, packet):
		field_name = self.layer_name + '.' + 'tim.dtim_period'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_tim_bmapctl(self, packet):
		field_name = self.layer_name + '.' + 'tim.bmapctl'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_tim_bmapctl_multicast(self, packet):
		field_name = self.layer_name + '.' + 'tim.bmapctl.multicast'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_tim_bmapctl_offset(self, packet):
		field_name = self.layer_name + '.' + 'tim.bmapctl.offset'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_tim_partial_virtual_bitmap(self, packet):
		field_name = self.layer_name + '.' + 'tim.partial_virtual_bitmap'
		value = packet[self.layer_name].get_field_value(field_name)
		str_value = str(value)
		return str_value

	def wlan_mgt_tim_aid(self, packet):
